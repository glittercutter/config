	...
	def app_default(self, c):
		...
		if f.extension is not None:
			...
			if f.extension in ('html', 'htm', 'xhtml'):
				return self.either(c, 'luakit', 'opera', 'jumanji',
						'firefox', 'elinks', 'lynx')
			if f.extension == 'swf':
		...
		if f.image:
			if c.mode in (11, 12, 13, 14):
				...
				return self.either(c, 'geeqie', 'sxiv', 'feh', 'eog', 'mirage')
...
CustomApplications.generic('opera', 'firefox', 'apvlv', 'evince',
		'zathura', 'gimp', 'mirage', 'eog', 'jumanji', 'luakit', 'geeqie', 'mplayer',
			flags='d', deps=['X'])
